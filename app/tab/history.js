...
function buildLocationInfo(report, type) {
  ...
  if (!localAndTime) {
    return type === 'lost' ? 'Local da última visualização não informado' : 'Local do achado não informado';
  }
  ...
  return (
    <View style={styles.cardItem}>
      ...
      <View style={styles.info}>
        ...
        <Text style={styles.text}>Marca: {report.brand || 'Não informada'}</Text>
        <Text style={styles.text}>Modelo: {report.model || 'Não informado'}</Text>
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.card}>
        <View style={styles.sectionHeader}>
          <Ionicons name="help-circle-outline" size={22} color="#F01259" />
          <Text style={styles.sectionTitle}>Histórico de perdidos</Text>
        </View>
        ...
        {lostReports.length === 0 ? (
          ...
          lostReports.map((report) => (
            <ReportCard key={report.id} report={report} type="lost" />
          ))
        )}
      </View>
      ...
      <View style={styles.card}>
        <View style={styles.sectionHeader}>
          <Ionicons name="alert-circle-outline" size={22} color="#F01259" />
          <Text style={styles.sectionTitle}>Histórico de achados</Text>
        </View>
        ...
        {foundReports.length === 0 ? (
          ...
          foundReports.map((report) => (
            <ReportCard key={report.id} report={report} type="found" />
          ))
        )}
